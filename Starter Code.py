            ...
            try:
                ...
                if 0 <= player_idx < len(players) and player_idx < len(current_stacks):
                    ...
                    dp = {
                        ...
                        "prev_bet_size": prev_bet_size,  # New feature
                ...
                for block in hand_blocks:
                    ...
                    if hand_text:
                        try:
                            ...
                            errors += 1
                            logging.warning(f"Error parsing hand in {filename}: {str(e)}")
...
def prepare_data(df):
    ...
    numeric_features = ["pot_size", "stack_to_pot_ratio", "effective_stack", "board_connectedness",
                        "is_preflop_aggressor", "bet_size", "bet_size_to_pot_ratio", "prev_bet_size", "player_agg_freq"]
    ...
    features = [
        ...
        "is_preflop_aggressor", "bet_size_to_pot_ratio", "prev_bet_size", "player_agg_freq"
    ]
    ...
    X_bet = X[cbr_mask]
    y_bet = y_bet[cbr_mask]
    
    return X, y_action, X_bet, y_bet, le_action.classes_

# --- Step 3: Model Training with Cross-Validation ---
def train_models(X, y_action, X_bet, y_bet, action_classes):
    ...
    params = {
        ...
        'max_depth': 6,           # Relaxed from 4
        'eta': 0.1,              # Relaxed from 0.05
        'subsample': 0.8,        # Relaxed from 0.7
        'colsample_bytree': 0.8, # Relaxed from 0.7
        'lambda': 2.0,           # Reduced from 5.0
        'alpha': 1.0,            # Reduced from 2.0
        'seed': 42
    }
    ...
    # Bet size regression with 5-Fold CV
    if len(X_bet) > 100:
        params_bet = {
        ...
        for train_idx, test_idx in kf.split(X_bet):
            ...
            y_bet_train, y_bet_test = y_bet.iloc[train_idx], y_bet.iloc[test_idx]
        ...
        print(f"5-Fold CV Bet Size MSE: {np.mean(bet_mse):.6f} (+/- {np.std(bet_mse) * 2:.6f})")
        
        # Train final bet size model
        X_bet_train_temp, X_bet_val, y_bet_train_temp, y_bet_val = train_test_split(X_bet, y_bet, test_size=0.2, random_state=42)
        dtrain_bet = xgb.DMatrix(X_bet_train_temp, label=y_bet_train_temp)
        dval_bet = xgb.DMatrix(X_bet_val, label=y_bet_val)
        reg = xgb.train(params_bet, dtrain_bet, num_boost_round=100, evals=[(dtrain_bet, 'train'), (dval_bet, 'val')], 
                        early_stopping_rounds=10, verbose_eval=False)
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            ...
            X, y_action, X_bet, y_bet, action_classes = prepare_data(df)
            clf, reg = train_models(X, y_action, X_bet, y_bet, action_classes)
