...
def parse_hand(hand_text):
    """
    Parses actions, storing lines like 'p1 sm Tc8d' (hole cards),
    and also storing board cards to 'board_cards' so Phase 2 can combine them.
    """
    try:
        hand = {}
        ...
        for action in actions:
            ...
            try:
                ...
                if 0 <= player_idx < len(players) and player_idx < len(current_stacks):
                    ...
                    dp = {
                        "street": street,
                        "board_cards": "|".join(board),  # <--- add board_cards
                        "pot_size": pot_size,
...
def process_folder(base_folder_path, output_csv):
    ...
    # Optional: print c-bet ratio distribution
...
def prepare_data(df):
    ...
    df["bet_size_to_pot_ratio"] = df["bet_size_to_pot_ratio"].clip(lower=0.0, upper=5.0)  # Cap at 500% of pot
